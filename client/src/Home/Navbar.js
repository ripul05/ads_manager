...
import { FaHome, FaAtom, FaNetworkWired, FaRocket, FaChartLine, FaSearch, FaCrosshairs, FaChevronDown, FaChevronUp, FaGoogle, FaFacebook , FaCode, FaDesktop } from "react-icons/fa";
...
const Navbar = ({ isContactPage = false, activeIndex=0, setActiveIndex }) => {
  ...
  // Create contact page navigation items (flattened services)
  const contactNavItems = useMemo(() => {
    const servicesItem = navItems.find(item => item.subItems);
    return [
      { label: "Home", href: "/", icon: <FaHome className="mr-2" /> },
      ...servicesItem.subItems,
    ];
  }, [navItems]);

  const currentNavItems = isContactPage ? contactNavItems : navItems;
  ...
  const handleNavClick = (index, e) => {
    const href = currentNavItems[index].href;
    e.preventDefault();
    
    // For contact page, handle navigation differently
    if (isContactPage) {
      navigate(href);
    ...
    if (currentNavItems[index].subItems) {
            ...
            <div className="flex items-center space-x-1 bg-black/40 backdrop-blur-sm rounded-full p-2 border border-cyan-400/20">
              {currentNavItems.map((item, index) => (
                <div 
                  ...
                  onMouseEnter={() => !isContactPage && item.subItems && setIsServicesHovered(true)}
                  onMouseLeave={() => !isContactPage && item.subItems && setTimeout(() => {
                ...
                >
                  ...
                  {/* Enhanced Services Dropdown for Desktop (only for non-contact pages) */}
                  {!isContactPage && item.subItems && (isServicesHovered || isDropdownHovered) && (
                    <motion.div
                      ref={dropdownRef}
              ...
              <div key={index}>
                <motion.a
                  ...
                  className={`block px-6 py-4 rounded-xl text-base font-medium flex items-center transition-all ${
                    activeIndex === index && (!item.subItems || isContactPage)
                ...
                {/* Enhanced Services Dropdown for Mobile (only for non-contact pages) */}
                {!isContactPage && item.subItems && isServicesClicked && (
                  <motion.div
